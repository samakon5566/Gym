...
//              calendarHeaderHtml+=    "</div>";
  ...
  TODO    if total height for an hour is less than 11px, there is a display problem.
      ...
      _renderCalendarBodyEvents: function($calendarTableTbody) {
        ...
        for (var i = start; i < end; i++) {
          ...
          if (options.use24Hour) {
              renderRow += '<div class=\"wc-time-header-cell\"><div class=\"wc-time-header-inner-cell\">' + self._24HourForIndex(i) + '</div></div>';
          }
          else {
      ...
      _loadCalEvents: function(dateWithinWeek) {
          ...
          //按鈕回傳不同參數        
          var keyword = $("#in_height").val();
          var moneyrange = $("#slider-range").slider("values");
          console.log(moneyrange);
          if (moneyrange.length == 1)
              moneyrange = [2000, 6000];
          $("#search").click(function () {
              keyword = $("#in_height").val();
              console.log(keyword);
              self._loadCalEvents();
          })
          $("#slider-range").live("slidechange", function () {
              moneyrange = $("#slider-range").slider("values");
              self._loadCalEvents();
          })
            $("#filterdetail>.order").live("click",function () {
                var detail = $("#drop>.order");
                var names = [];
                ...
                $(".drop>.btn>.fa-times").click(function () {
                    var changdetail = $("#drop>.order");
                    names = [];
                    changdetail.each(function (index, value) {
                        names.push($(value).text());                  
                    })
                    $(".storage").val(names);
                    self._loadCalEvents();
          ...
          function LoadLessons() {
              var calevent = $(".wc-cal-event");
              calevent.click(function () {
                  var ClassName = $(this).find(".wc-title").text();
                  var url = '/GroupCourse/ShowLessons?name=' + ClassName;
                  console.log(url);
                  // todo1.修改成用frag裝內部資料
                  $.getJSON(url, (data) => {
                      const List = $("ul.list-group");
                      List.empty();
                      $.each(data[0].lesson, function (index, value) {
                          formatTime = value.replaceAll("T", " ");
                          var lesson = $("<li></li>").addClass("list-group-item").text(formatTime);
                          List.append(lesson);
                      })
                      //班別資訊   
                      $("#Purchase-img").attr("src", `/img/CourseDetail/${data[0].classpictrue}`);
                      $("#detail").text(data[0].detailname)
                      $("#classID").val(data[0].classID);
                      $("#coach").text(data[0].coach);
                      $("#class").text(data[0].classname);
                      if (data[0].actuallymoney != data[0].money)
                          $("#money").html(`NT$${data[0].actuallymoney}<s>    NT$${data[0].money}</s>`);
                      else
                          $("#money").html(`NT$${data[0].money}</s>`);
                      console.log(data[0].classpeople,data[0].vacancy)
                      $("#project").text(data[0].dicount);                      
                      if (data[0].classpeople == 0) {
                          $("#vacancy").css({ "fontSize": "20px" }).text("已額滿");
                          $(".ui-button").eq(4).attr({ "aria-disabled": "true", "disabled": "disabled" });
                      }                      
                      else if (data[0].classpeople / data[0].vacancy >=0.5)
                          $("#vacancy").text(`剩餘名額:${data[0].classpeople}/${data[0].vacancy}人`).css({ "color":"white","animation":"none","transition":"unset"});
                      else
                          $("#vacancy").css({ "color": "red", "fontSize": "20px", "animation": "big 3s linear infinite", "transition": "1.5s" }).text(`剩餘:${data[0].classpeople}/${data[0].vacancy}人,要搶要趕緊`);
                  })
              })
          }
          var value = $(".storage").val().split(",");          
          var parameter = Array.from(value);
          var jsonOptions = self._getJsonOptions();
              jsonOptions[options.startParam || 'start'] = Math.round(weekStartDate.getTime() / 1000);
              jsonOptions[options.endParam || 'end'] = Math.round(weekEndDate.getTime() / 1000);
          _currentAjaxCall = $.ajax({
              url: "/GroupCourse/Showclass",
              data: { id: parameter, "keyword": keyword, "moneyrange": moneyrange } ,
              dataType: 'json',
              traditional:true,
              error: function(XMLHttpRequest, textStatus, errorThrown) {
                ...
                success: function (data) {
                    ...
                    LoadLessons();
                    $(".wc-prev,.wc-next").click(function () {
                        LoadLessons();
                    })
              },
              complete: function() {
      ...
      _updateDayColumnHeader: function($weekDayColumns) {
          ...
          var showAsSeparatedUser = options.showAsSeparateUsers && options.users && options.users.length;
          var todayClass = 'ui-state-active ui-state-active2 wc-today';
